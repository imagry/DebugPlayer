...
def get_interpolated_pose_SE2(P_time_stamp, yaw, time_stamp_yaw, ego_x, ego_y, ego_time_stamp):
    ...
    if False:
        # For debug purposes, plot ego and interpulated ego together
        with plt.ioff():
        # fig, ax = plt.subplots(2,2,)
            fig, (ax1) = plt.subplots(1, 2, figsize=(5, 2.7))
            my_plotter(ax1, ego_x, ego_y, {'color':'orange', 'linewidth':1, 'linestyle':'--','label':'ego'})
            my_plotter(ax1, interpolated_ego_x, interpolated_ego_y, {'color':'blue', 'linewidth':1, 'linestyle':'--','label':'Interpolated_ego'})
            # my_plotter(ax3, X_gps_lng_lat[0,:], X_gps_lng_lat[1,:], {'color':'blue','linewidth':1, 'linestyle':'-','label':'gps'})
